...
export function ValueProvider({ children }) {
  // 3-1 使用useState建立一個狀態變數valueFormContext 與狀態更新函式setValueFormContext
  //並在等號的右邊 useState 設置初始值
  // 因為這個範例要做數字計算 所以初始值設為0
  // 但這個初始值可以是任何資料型態
  const [valueFormContext, setValueFormContext] = useState(0)
  ...
  // 如果有複數個狀態變數就一起放入
  return (
    <exampleContext.Provider value={{ valueFormContext, setValueFormContext }}>
      {children}
...
// 上面設置好的狀態變數valueFormContext 與狀態更新函式setValueFormContext 就會變成全域可使用的

// ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
// 4.建立第二個function 這個function在React的結構中算是一種Hooks 取名use開頭 模仿useState這種原生Hooks的取名方式，
// 這裡因為我是要取用valueFormContext這個狀態變數 所以我將Hooks取名為useValueFormContext
// 這個自己取名的Hooks是要使用於要取出 狀態變數 及 狀態更新函式 的頁面

export const useValueFormContext = () => {
